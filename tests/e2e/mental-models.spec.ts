...
test.describe('every model teaches prediction, evidence, failure reasoning, transfer, and replay', () => {
  for (const model of models) test(model.title, async ({ page }) => {
    ...
    for (let index = 0; index < model.steps.length; index += 1) {
      ...
      const wrong = step.prediction.options.find((option: { id: string }) => option.id !== step.prediction.answer_id);
      await workspace.getByRole('radio', { name: wrong.text }).check();
      await workspace.getByRole('button', { name: 'Check prediction & reveal' }).click();
      await expect(workspace.getByRole('heading', { name: 'Let’s correct the model' })).toBeVisible();
      await expect(workspace.getByRole('status')).toContainText(step.action);

      const actor = model.components.find((component: { id: string }) => component.id === step.actor_id);
      await workspace.getByRole('button', { name: new RegExp(actor.name) }).click();
      await expect(workspace.getByText(actor.responsibility, { exact: true })).toBeVisible();

      await workspace.getByText('Inspect the evidence', { exact: true }).click();
      await expect(workspace.getByText(step.proof.command, { exact: true })).toBeVisible();
      const evidence = workspace.locator('details.mental-model-evidence');
      await expect(evidence).toContainText(step.proof.limitation);

      const failureToggle = workspace.getByRole('checkbox', { name: new RegExp(step.failure.condition.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) });
      await failureToggle.check();
      const nextCheck = workspace.locator('.mental-model-feedback p').filter({ hasText: 'Next check:' });
      await expect(nextCheck).toContainText(step.failure.next_check);
      await failureToggle.uncheck();

      if (index + 1 < model.steps.length) {
        await workspace.getByRole('button', { name: 'Predict next step' }).click();
      }
    }

    for (const transfer of model.transfer_questions) {
      await expect(workspace.getByText(transfer.prompt, { exact: true })).toBeVisible();
    }

    await workspace.getByRole('button', { name: 'Replay scenario' }).click();
    await expect(workspace.getByText(`1 of ${model.steps.length} steps`, { exact: false })).toBeVisible();
    await expect(workspace.getByRole('button', { name: 'Check prediction & reveal' })).toBeDisabled();
  });
});
